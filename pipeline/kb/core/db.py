...
# migrations 保留在 kb/ 下不动：backend TS 按 ../../pipeline/kb/migrations 硬编码读取（backend/src/db.ts），
# 是跨栈契约路径，随本模块下沉会让后端读不到 schema。
MIGRATIONS_DIR = Path(__file__).resolve().parent.parent / "migrations"
_ENV_FILE = Path(__file__).resolve().parents[2] / ".env"  # core/ -> kb/ -> pipeline/
